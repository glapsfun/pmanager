...
| `check [--json] [--stale-days N] [--epic <slug>]` | Phase 6 before commit; every update run; after any hand edit; pass `--epic <slug>` during an epic's run so it reads that epic's worktree | exit 0, `OK: no findings` or warnings only | exit 1: fix each finding at its file, rerun; never commit with errors; never edit rendered regions by hand |
| `render [--epic <slug>]` | after writing or editing any epic/task/log file; after pulling `main`; pass `--epic <slug>` during an epic's run so it writes in that epic's worktree | exit 0; prints rendered paths or `nothing to render` | never fails on valid input; if it writes something unexpected, run `check` |
...
| `claim <slug> --harness NAME [--no-worktree]` | Phase 4, right after the user approves the epic and `epic.md` is written | exit 0; the epic's worktree is created at `<repo>-pm-<slug>` and named on the `worktree:` line, the epic files you wrote are moved there, and the branch is pushed; your own checkout keeps its branch | exit 1 `owned by …`: stop, tell the user who owns it; exit 2: no remote, no epic file, or `worktree`: the sibling path is occupied, the branch is checked out elsewhere, or `git worktree add` failed — move it aside or pass `--no-worktree` |
...
and `render --epic <slug>` / `check --epic <slug>` act inside it. You never
`cd` for those commands — the tool resolves the worktree from git — but the
Phase 6 commit is yours to run, so point git at the worktree (`git -C`). Your
own checkout keeps whatever branch it was on. `--no-worktree` restores the
old behaviour of switching the current checkout; use it only when a worktree
cannot be created.
...
3. `pm render --epic <slug>`, then `pm check --epic <slug>`; fix findings;
   repeat until exit 0. Write every document inside the worktree `claim`
   named.
4. Commit in that worktree (`W` below is the path `claim` printed):
   `git -C W add docs/pm && git -C W commit docs/pm -m "docs(pm): <what>"`.
5. `git -C W push origin pm/<slug>` — the one push the skill may do.
